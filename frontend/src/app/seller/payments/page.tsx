...
export default function SellerPaymentsPage() {
  ...
  return <div className="space-y-6"><div><h1 className="text-3xl font-bold">Payments</h1><p className="text-gray-600">Payments belonging to your events.</p></div><DataTable columns={columns} data={data?.data ?? []} totalCount={data?.pagination.total ?? 0} currentPage={pagination.page} pageSize={pagination.pageSize} onPaginationChange={setPagination} title="Payments" enablePagination loading={isLoading} emptyMessage={error ? "Failed to load payments" : "No payments found"} /></div>;
}
