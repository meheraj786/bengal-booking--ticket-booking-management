  ...
  return (
    <aside className="w-full rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="divide-y divide-slate-100">
        ...
        {/* ── 3. Location (searchable combobox) ── */}
        <div className="px-5 py-4 space-y-3">
          <div className="flex items-center gap-2">
          ...
          <Popover open={locationOpen} onOpenChange={setLocationOpen}>
            <PopoverTrigger >
              <button
                type="button"
                className={cn(
                  "w-full flex items-center justify-between gap-2 h-9 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium hover:border-primary/50 hover:bg-primary/5 transition-all",
                  selectedArea !== "all" ? "text-slate-800" : "text-slate-400",
                )}
              >
                <span className="truncate">
                  {selectedArea !== "all"
                    ? (selectedAreaObj?.name ?? selectedArea)
                    : "All locations"}
                </span>
